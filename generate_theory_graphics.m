...
grid on;
title('Subamostragem (M=2): Remoção de Amostras');
xlabel('Amostras (n)'); ylabel('Amplitude');
ylim([-1.2, 1.2]);
...
f_alias = 12; % Original High Frequency Signal (12 Hz)
f_fund = 2;   % Aliased Low Frequency perceived (2 Hz)
...
plot(t_cont, y_alias_cont, 'r--', 'LineWidth', 1.2); hold on;
plot(t_cont, y_fund_cont, 'b', 'LineWidth', 2);
stem(t_samp, y_samp, 'k', 'filled', 'LineWidth', 1.5, 'MarkerSize', 8);
grid on;

title('Efeito de Aliasing: Múltiplas frequências gerando as mesmas amostras digitais');
xlabel('Tempo (s)'); ylabel('Amplitude');
legend('Sinal Original (12 Hz)', 'Sinal com Aliasing (2 Hz)', 'Amostras Digitais (fs = 10 Hz)', 'Location', 'northeast');
ylim([-1.5, 1.5]);
